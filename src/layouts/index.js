...
class TemplateWrapper extends Component {
  ...
  navigateScroll = e => {
    const {deltaY} = e;
    if(deltaY>0) this.navigate({ keyCode: this.NEXT[0] });
    else this.navigate({ keyCode: this.PREV });
  };

  componentDidMount() {
    document.addEventListener('keydown', this.navigate);
    document.addEventListener('wheel', this.navigateScroll);
  }

  componentWillUnmount() {
    document.removeEventListener('keydown', this.navigate);
    document.removeEventListener('wheel', this.navigateScroll);
  }
